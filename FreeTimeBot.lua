...
function automaticUpdates()
  if autoUpdate == true then
    if currentVersion == latestVersion then
      ...
      scriptExit("You have Updated FreeTimeBot!")
    end
  end
end
...
noLeaderSkill = false
riftRaidDefeat = true
raidBattleTime = false
miniBoss = false
autoUpdate = true
end
function defaultRegionLocation()
miniBossChatsymbolRegion = Region(40, 125, 85, 69)
dungeonStage3Region = Region(852, 499, 100, 200)
miniBossRegion = Region(21, 10, 121, 199)
friendLeaderRegion = Region(403, 253, 166, 68)
...
stoneRarityRegion = Region(723, 363, 31, 28)
...
raidVictoryTotalRegion = Region(1394, 586, 116, 61)
...
leavePartyYesRegion = Region(740, 620, 105, 65)
...
rechargeFlashRegion = Region(675, 391, 241, 220)
...
liveArenaChoiceMonRegion = Region(25, 719, 1799, 333)
liveArenaChoiceLeaderRegion = Region(833, 577, 652, 413)
...
function showBattleToast(duration)
  ...
  local totalShopCount = searchMagicShopCount + mysticalCount + lightDarkCount + legendaryCount
  local totalStoneCount = LegStoneCount + HeroStoneCount + RareStoneCount  
  if runNum then
...
6* save:]] .. r6Count .. "  5* save:" .. r5Count .. "  6* sold:" .. r6Sold .. "  5* sold:" .. r5Sold .. "  Runes sold:" .. totalRuneSold
  ...
  if totalShopCount > 0 then
    toastMessage = toastMessage .. [[

Shop:]] .. searchMagicShopCount .. " MS:" .. mysticalCount .. " LD:" .. lightDarkCount .. " LS:" .. legendaryCount
  end
  if totalStoneCount > 0 then
    toastMessage = toastMessage .. [[

Stones: Leg:]] .. LegStoneCount .. "  Hero:" .. HeroStoneCount .. "  Sold:" .. RareStoneCount
  end	  
  toast(toastMessage)
  toast(toastMessage)
  toast(toastMessage)
...
function printBattleMessage()
  local stats = "Elapsed Time: "..SecondsToClockHour(elapsedTimeT:check())
  local message = ""
  local totalBattle = riftBattle + loseCount + winCount
  local totalArenaBattle = arenaWinCount + arenaLoseCount
  local totalRuneCount = runeSave + r6Count + r6Sold + r5Count + r5Sold + runeSold
  ...
  local totalShopCount = searchMagicShopCount + mysticalCount + lightDarkCount + legendaryCount 
  local totalStoneCount = LegStoneCount + HeroStoneCount + RareStoneCount 
  local totalTime = totalTimeCount  
  if totalBattle > 0 then
...
Elapsed Time: ]] ..SecondsToClockHour(elapsedTimeT:check())
  end    
  if message ~= "" then
    message = message .. [[
...
function showBattleResult(command) ----- right on screen (screenStats)
  local message = ""
  local totalBattle = riftBattle + loseCount + winCount
  local totalArenaBattle = arenaWinCount + arenaLoseCount
  local totalRuneCount = runeSave + r6Count + r6Sold + r5Count + r5Sold + runeSold
...
Leg:]] .. LegStoneCount .. "  Hero:" .. HeroStoneCount .. "  Sold:" .. RareStoneCount
  end   
  if runArena then 
	message = message .. [[
...
Arena:]] .. totalArenaBattle .. "  W:" .. arenaWinCount .. "  L:" .. arenaLoseCount
  ...
  if runGiant or runDragon or runNecro then
    message = message .. [[
...
function dialogBox()
  ...
  newRow()
  addTextView("'Dungeon level' and 'Target MiniBoss' only applies to Carios Dungeon.")
  addTextView("  ")
  addSeparator()
  newRow()
  ...
  addTextView("  ")
  addCheckBox("runMagicShop", "Check Magic Shop?", false)
  addTextView("  ")
  addCheckBox("miniBoss", "Target MiniBoss?", false)
  addSeparator()
  newRow()
  addCheckBox("runFriendMon", "Use Friend Mon in Scenario?", false)
  newRow()
  addCheckBox("isSwitchFodderSlot3", "Use RIGHT", false) addCheckBox("isSwitchFodderSlot4", "and BOTTOM slot in Scenario?", false)
  newRow()
  spinnerBattleLimit = {
    "Infinite Runs",
    "Custom Runs",
  ...
  addSeparator()
  newRow()
  addCheckBox("refillEnergy", "Refill Energy with Crystals?", false)
  addTextView("  ")
  ...
  addTextView("    ")
  addSpinner("maxDetectPct", spinnerMaxDetectPct, spinnerMaxDetectPct[6])
  addTextView("% MaxLevel Accuracy")
  newRow()
  addSpinner("textSize", spinnerTextSize, spinnerTextSize[4])
  ...
  dialogShowFullScreen("FreeTimeBot Config:")
end
...
function setDialogOptions()
  ...
  elseif runLmtOption == spinnerBattleLimit[8] then
    stopMaxLevel = true
  elseif runLmtOption == spinnerBattleLimit[9] then
...
function setAdvancedOptions()
  ...
  maxLevelAccuracy = maxDetectPct * 0.01
...
function start()
  if startRegion:exists(Pattern("start.png"):similar(imgAccuracy), 2) then
	...
	if exists(Pattern("alreadyUsed.png"):similar(.7), 0.1) then
	  soundVibrate("victory.mp3", false)
	  scriptExit("Friends list are over")
    end
  end
  clearSnapshot()
end
...
function attackMiniBoss()
	...
	if (miniBoss == true and miniBossDead == false) then
      ...
      until(dungeonStage3Region:exists(Pattern("DungeonStageThree.png"):similar(.8), 0.1) or miniBossChatsymbolRegion:exists(Pattern("Chatsymbol.png"):similar(.8), 0.1))
      wait(1.4)
	  toast("Attack the MiniBoss")
	  click(Location (1100, 240, 230, 200))
	  wait(30)
      miniBossDead = true 	
      end
    end
...
function refill()
  ...
  if exists(Pattern("notEnoughtCrystals.png"):similar(.7), 0.1) then
	...
	keyevent(4)
    while waitTimer > 0 do
      showCommand("Out of Crystals.  Waiting " .. waitTimer .. " minutes.")
      wait(60)
      waitTimer = waitTimer - 1
      checkMagicShop()
    end
...
function getBattleSlotStarLevel(slot)
  ...
  elseif slot == 3 then
  local accuracy = starAccuracy * 0.86
...
function getBattleSlotLevel(slot)
  local accuracy = maxLevelAccuracy * 0.99
  if slot == 1 then
...
function checkMaxLevel()
  if stopMaxLevel then
    if runFriendMon and slot1MaxLevelRegion:exists(Pattern("maxLevel.png"), 2) then
	  isMaxLevel = true
	end  
...
function friendMon()
  if runFriendMon then
    click(Location (412, 217, 149, 140))
    wait(2)
    click(Location (49, 875, 128, 126))
  end
end
-------------------------------------------------------------------------------------------------------------------------------------
...
function stoneKeep()
  if stoneRarity == 0 then
    raidOkRegion:existsClick(Pattern("ok.png"):similar(0.6), 0.1)
  else 
    if stoneRarity == 6 then 
      getStone()
	else 
      if stoneRarity == 5 then
        getStone()
	  else 
	    if stoneRarity == 4 then
          sellStone()
		end
	  end
	end 
...
function sellStone()
  ...
  if stoneRarity == 4 then RareStoneCount = RareStoneCount + 1
  toast("Sell")
  end
end
...
function liveArenaRepeatedMon()
  keyevent(4)
  liveArenaChoiceMonRegion:existsClick(Pattern("icares.png"):similar(0.6), 0.1) 
  liveArenaTopRegion:existsClick(Pattern("liveArenaOk.png"):similar(runeAccuracy), 0.1) 
  liveArenaChoiceMonRegion:existsClick(Pattern("chow.png"):similar(0.6), 0.1)
  liveArenaTopRegion:existsClick(Pattern("liveArenaOk.png"):similar(runeAccuracy), 0.1) 
end
function liveArenaLeader()
  wait(5)
  toast("Selecting Leader")
  existsClick(Pattern("moLong.png"):similar(0.6), 0.1)
  liveArenaTopRegion:existsClick(Pattern("liveArenaOk.png"):similar(runeAccuracy), 0.1)
  end
function liveArenaBanOpp()    
...
function closeRiftInfo()
  if raidDropInfoRegion:exists(Pattern("raidDropInfo.png"), 2) then 
    keyevent(4)
	toast("Close Info")
  end	
end
...
function findRift()
  ...
  elseif exists(Pattern("mapRiftOfWorlds.png"), 0.1) then
    ...
    clickRift()
	runRiftRaidStart()
  elseif raidJoinRegion:exists(Pattern("raidJoinParty.png"):similar(imgAccuracy), 0.1) then
    raidReadyRegion:existsClick(Pattern("raidReady.png"):similar(0.6), 0.1)
    runRiftRaidStart()
  elseif raidReadyRegion:exists(Pattern("raidReady.png"):similar(imgAccuracy), 0.1) then
    runRiftRaidStart()
  elseif arenaRankRegion:exists(Pattern("arenaRank.png"):similar(imgAccuracy), 0.1) then
    closeArenaDialogBox()
    enterRift()
...
function getIslandStuff()
  ...
  local a = 4
  if not exists(Pattern("islandMana.png"), 0.1) or exists(Pattern("islandMana1.png"), 0.1) then
    while a > 0 do
      zoomOut()
      wait(1)
      a = a - 1
  ...
  a = 3
  while a > 0 and exists(Pattern("islandMana.png"), 0.1) or exists(Pattern("islandMana1.png"), 0.1) do
    clickIslandMana()
    do break end
    do break end
    a = a - 1
...
function purchaseMagicShopScroll()
  ...
  while a > 0 do
    ...
    if magicShopItemRegion:exists(Pattern("summoningStone.png"), 1) then
      magicShopItemRegion:existsClick(Pattern("summoningStone.png"), 1)
      existsClick(Pattern("buyMagicShop.png"), 1)
      existsClick(Pattern("yesMagicShop.png"), 1)
    end
    if magicShopItemRegion:exists(Pattern("unknownScroll.png"), 1) then
      magicShopItemRegion:existsClick(Pattern("unknownScroll.png"), 1)
      existsClick(Pattern("buyMagicShop.png"), 1)
      existsClick(Pattern("yesMagicShop.png"), 1)
    end		
    wait(1)
...
function closeIslandAds()
  if spamAdsRegion:exists(Pattern("spamAds1.png"):similar(.7), 0.1) then
    toast("Closing ADS")
	keyevent(4)
  end
  if spamAdsRegion:exists(Pattern("spamAds2.png"):similar(.7), 0.1) then
    toast("Closing ADS")
	keyevent(4)
  end
  if exists(Pattern("selectMon.png"):similar(.7), 0.1) then
    toast("Closing ADS")  
	keyevent(4)
	keyevent(4)
  end
...
function findMagicShop()
  ...
  if liveArenaBattleRegion:exists(Pattern("liveArenaBattle.png"), 0.1) then
	...
	closeIslandAds()
    getIslandStuff()  
  elseif arenaRankRegion:exists(Pattern("arenaRank.png"), 0.1) then
    closeArenaDialogBox()
    backButtonRegion:existsClick(Pattern("backButton.png"):similar(0.8), 2)
	closeIslandAds()
    getIslandStuff()
  ...
  elseif dialogCairoDungeonRegion:exists(Pattern("cairoDungeon.png"):similar(imgAccuracy), 0.1) then
	...
	closeIslandAds()
	getIslandStuff()
  elseif sameSessionRegion:exists(Pattern("sameSession.png"):similar(0.6), 2) then
    keyevent(4)
    closeRiftDialogBox()
    clickRiftBack()
    wait(1)
    backButtonRegion:existsClick(Pattern("backButton.png"):similar(0.8), 2)
    wait(1)
	if exists(Pattern("closePopUpNow.png"):similar(imgAccuracy), 0.1) then
      toast("Closing ADS")	 
	  yesRegion:existsClick(Pattern("yes.png"):similar(imgAccuracy * 0.9), 3)
	  backButtonRegion:existsClick(Pattern("backButton.png"):similar(0.8), 2)
	end
...
function runLiveArenaStart()
  while runLiveArena do
    if battleIconRegion:exists(Pattern("iconBattle.png"), 2) then
	  battleIconRegion:existsClick(Pattern("iconBattle.png"), 2)
	end
    if liveArenaBattleRegion:exists(Pattern("liveArenaBattle.png"):similar(imgAccuracy), 2) and runMagicShop == true then
      checkMagicShop()
	end  
    if liveArenaBattleRegion:exists(Pattern("liveArenaBattle.png"), 0.1) then
      liveArenaBattleRegion:existsClick(Pattern("liveArenaBattle.png"), 0.1)
    end
	if exists(Pattern("notEnoughTickets.png"):similar(.7), 0.1) then
	  toast("Not enough Tickets")
	  soundVibrate("victory.mp3", false)
	  scriptExit("Tickets are over")
    end	
	...
	if exists(Pattern("liveArenaCantHave.png"):similar(.7), 0.1) then
	  liveArenaRepeatedMon()
	end
	if liveArenaTopRegion:exists(Pattern("liveArenaBanOpp.png"), 0.1) then
      liveArenaBanOpp()
      liveArenaTopRegion:existsClick(Pattern("liveArenaOk.png"), 0.1)
	...
	if exists(Pattern("startSpam.png"):similar(.7), 0.1) then
      existsClick(Pattern("startSpam.png"):similar(0.9), 1)
	end  
	if exists(Pattern("startLogo.png"):similar(.7), 0.1) then
	  existsClick(Pattern("startLogo.png"):similar(0.9), 1)
	end  
    if spamAdsRegion:exists(Pattern("spamAds1.png"):similar(.7), 0.1) or spamAdsRegion:exists(Pattern("spamAds2.png"):similar(.7), 0.1) then
	  toast("Closing ADS")
	  keyevent(4)
	end	
	if exists(Pattern("giftBox.png"):similar(.7), 0.1) then
	  toast("Closing Gift Box")	
      keyevent(4)
	  findLiveArena()
	end
    if liveArenaTopRegion:exists(Pattern("liveArenaSelect.png"), 0.1) then
	...
	if exists(Pattern("liveArenaUnderMaint.png"):similar(.7), 0.1) then
	  toast("Live Arena does not work")
	  soundVibrate("alarm.mp3", false)
	  scriptExit("The Live Arena is under maintenance")
    end	  
  end
end
checkNoRaidActivity = true
...
function runRiftRaidStart()
  while runRiftRaid do
    ...
    if noLeaderSkillRegion:exists(Pattern("noLeaderSkill.png"):similar(0.6), 0.1) and noLeaderSkill == true then
	  toast("No leader skill. Leaving party")
      keyevent(4)
	  keyevent(4)
	  leavePartyYesRegion:existsClick(Pattern("yes.png"):similar(0.6), 0.1)
      wait(5)
    elseif noLeaderSkillRegion:exists(Pattern("noLeaderSkill.png"):similar(0.6), 0.1) and noLeaderSkill == false then 	
	  toast("No leader skill. Continue")
	  noLeaderSkillYesRegion:existsClick(Pattern("yes.png"):similar(0.6), 0.1)
	end
	raidReadyRegion:existsClick(Pattern("raidReady.png"):similar(0.6), 0.1)	
	if exists(Pattern("raidPlaying.png"):similar(.7), 0.1) then
	  wait(10)
	end
	raidReadyRegion:existsClick(Pattern("raidReady.png"):similar(0.6), 0.1)	
	if exists(Pattern("everyNotReady.png"):similar(.7), 0.1) then
	  toast("Leaving party")	
      keyevent(4)
	  keyevent(4)
    end
	raidReadyRegion:existsClick(Pattern("raidReady.png"):similar(0.6), 0.1)	
	if exists(Pattern("youNeedAtLeast.png"):similar(.7), 0.1) then
	  toast("Leaving party")	
      keyevent(4)
	  keyevent(4)
    end
	raidReadyRegion:existsClick(Pattern("raidReady.png"):similar(0.6), 0.1) 	
    if exists(Pattern("leaveParty.png"):similar(.7), 0.1) then
	 leavePartyYesRegion:existsClick(Pattern("yes.png"):similar(0.6), 0.1)
    end
    ...
    if raidVictoryTotalRegion:exists(Pattern("raidVictoryTotal.png"):similar(0.6), 0.1) then
      ...
      if sameSessionRegion:exists(Pattern("sameSession.png"):similar(0.6), 2) then
        ...
        if raidBattleTimeRegion:exists(Pattern("raidBattleTime4.png"):similar(0.6), 2) and raidBattleTime == true then
		  toast("The battle lasted more than 4 minutes. Leaving party")
		  keyevent(4)
        elseif raidBattleTimeRegion:exists(Pattern("raidBattleTime4.png"):similar(0.6), 2) and raidBattleTime == false then 
          refillYesRegion:existsClick(Pattern("yes.png"):similar(0.6), 0.1)		
		end
      end	  
    end
    if raidLossTotalRegion:exists(Pattern("raidVictoryTotal.png"):similar(0.6), 0.1) then
      ...
      if stopDefeat == true then
	     soundVibrate("defeat.mp3", false)
         scriptExit ("Defeated, stopping as requested!")
      end
      wait(3)
      if sameSessionRegion:exists(Pattern("sameSession.png"):similar(0.6), 2) and riftRaidDefeat == true then
      toast("Defeated. Leaving party")	  
      keyevent(4)
	  elseif sameSessionRegion:exists(Pattern("sameSession.png"):similar(0.6), 2) and riftRaidDefeat == false then	
      refillYesRegion:existsClick(Pattern("yes.png"):similar(0.6), 0.1)	
      end	  
    end
    raidOkRegion:existsClick(Pattern("ok.png"):similar(0.6), 0.1)
    ...
    if exists(Pattern("riftRaid.png"):similar(0.6), 0.1) then	
	  existsClick(Pattern("riftRaid.png"):similar(.6), 3)
	end 
...
while true do
  ...
  elseif not runLiveArena or not runQuickClick or not runRiftRaid then
    ...
    if victoryDefeatRegion:exists(Pattern("defeat.png"):similar(imgAccuracy * 1.1), 0.3) or reviveNoRegion:exists(Pattern("noRevive.png"):similar(imgAccuracy), 0.1) then
      loseCount = loseCount + 1
      runLmt = runLmt - 1
      showBattleResult("Battle Start")
      printBattleMessage()
